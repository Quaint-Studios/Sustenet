...
type PacketHandler = fn(from_client: u32, packet: u32);
...
impl BaseServer {
    ...
    async fn add_client(&mut self, client: &TcpStream) -> Result<(), BaseServerError> {
        ...
        if id != None {
            self.disconnect_client(id.unwrap());
        }
    ...
    pub async fn on_received_data(&mut self, id: u32, data: &[u8]) {
