...
mod tests {
    ...
    #[test]
    fn test_mio_stress_test() {
        let server = std::thread::spawn(|| {
            let _ = super::main(); // Run the mio server in a separate thread
        });

        let max_conns = super::MAX_CONNS;
        let num_threads = 8;
            ...
            match event.token() {
                SERVER =>
                    loop {
                        ...
                        // print every 10000 connections
                        if connections.len() % 1 == 0 {
                            let elapsed = start_time.elapsed();
                            println!("Accepted {} connections in {:?}", connections.len(), elapsed);

                            if connections.len() >= (MAX_CONNS as usize) {
                                println!("Reached maximum connections: {}", MAX_CONNS);
                                return Ok(());
                            }
                        }
                    }
