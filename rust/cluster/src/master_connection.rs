...
use sustenet_shared::packets::{ ClusterSetup, Connection, Diagnostics }; // TODO: Add Messaging
...
impl MasterConnection {
    pub async fn connect(address: &str, port: u16) -> io::Result<Self> {
        ...
        LOGGER.info(&format!("Connecting to the master server at {addr}..."));
        ...
        let (event_tx, _event_rx) = broadcast::channel::<ClusterEvent>(16);
            ...
            lselect!(
                ...
                command = reader.read_u8() => {
                    match command {
                        ...
                        Err(e) => {
                            LOGGER.error(&format!("Failed to read command: {e}"));
                        }
                    }
                }
            );
    ...
    async fn handle_command(
        ...
        _reader: &mut BufReader<ReadHalf<'_>>,
        _writer: &mut WriteHalf<'_>,
